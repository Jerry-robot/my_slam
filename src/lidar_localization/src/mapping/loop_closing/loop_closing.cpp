...
LoopClosing::LoopClosing() {
    InitWithConfig();
    LOG(INFO) << "InitWithConfig finish!";
}
...
bool LoopClosing::InitParam(const YAML::Node& config_node) {
    ...
    LOG(INFO) << "extend_frame_num: " << extend_frame_num_ << std::endl
              << "loop_step: " << loop_step_ << std::endl
              << "diff_num: " << diff_num_ << std::endl
              << "detect_area: " << detect_area_ << std::endl
              << "fitness_score_limit: " << fitness_score_limit_ << std::endl
              << std::endl;
...
bool LoopClosing::Update(const KeyFrame key_frame, const KeyFrame key_gnss) {
    ...
    if (!DectectNearestKeyFrame(key_frame_index)) {
        return false;
    }
    LOG(INFO) << "DectectNearestKeyFrame FINISH!";

    if (!CloudRegistration(key_frame_index)) {
        return false;
    }
    LOG(INFO) << "CloudRegistration FINISH!";

    has_new_loop_pose_ = true;
    return true;
}

bool LoopClosing::DectectNearestKeyFrame(int& key_frame_index) {
    static int skip_cnt = 0;
    // skip_num:阈值（隔多少帧闭环检测）
...
bool LoopClosing::CloudRegistration(int key_frame_index) {
    ...
    loop_close_cnt++;
    LOG(INFO) << "检测到闭环 " << loop_close_cnt << ": 帧" << current_loop_pose_.index0 << "------>"
...
bool LoopClosing::JoinScan(CloudData::CLOUD_PTR& scan_cloud_ptr, Eigen::Matrix4f& scan_pose) {
